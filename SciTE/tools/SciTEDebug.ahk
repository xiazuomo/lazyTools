...
VL_Create()
{
	...
	Gui 4:Add, ListView, x0 y0 w320 h240 gVL_Inspect vVL_Listview, Scope|Variable name|Contents (partial)|空
	VL_Update()
	Gui 4:Show, w320 h240, Variable list
	LV_ModifyCol(1, "AutoHdr")
	LV_ModifyCol(2, "AutoHdr")
	LV_ModifyCol(3, "AutoHdr")
	LV_ModifyCol(4,0) ;第四行是个隐藏行，填充编号，用于支持彩色LV
...
VL_Update()
{
	...
	LV_Delete()
	LV_ColorChange()  ;重设颜色
	LV_ColorInitiate(4)   ;初始化
	Loop, % VL_Local.MaxIndex()
	{
                行++
                LV_Add("", "Local", VL_Local[A_Index], VL_LocalCont[A_Index], 行)
                LV_ColorChange(行,0x807153,0xFFE6C5,"I","I",4,0)   ;本地变量的颜色。
	}
	Loop, % VL_Global.MaxIndex()
	{
                行++
                LV_Add("", "Global", VL_Global[A_Index], VL_GlobalCont[A_Index], 行)
                LV_ColorChange(行,0x807153,0xFFF7EF,"I","I",4,0)   ;全局变量的颜色。
	}
	LV_Redraw()   ;全部行设置完毕，再刷新颜色
	ToolTip
}
...
;}
#Include %A_ScriptDir%\Lib\彩色LV.ahk