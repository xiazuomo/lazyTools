...
*/
; Release version
SetWorkingDir,%A_ScriptDir%	;¹¤×÷Ä¿Â¼±ØÐëÎª½Å±¾Ä¿Â¼
If (A_IsUnicode=1)
  {
    MsgBox, 4112, ´íÎó, ±ØÐëÊ¹ÓÃAnsi°æ±¾ÔËÐÐ£¡, 5
    ExitApp
  }
...
IfEqual, i, GiveMeSource
	...
	FileInstall, lib\func_Balloon2.ahk, %A_ScriptDir%\source %version%\lib\func_Balloon2.ahk, 1
...
iconlocal = res\icon.ico	; icon for uncompiled scripts
...
ModHome := "http://www" Chr(46) "autohotkey" Chr(46) "net/~Drugwash/smartgui"Chr(46) "html"
RMail := "mr" Chr(46) "rajat" Chr(64) "gmail" Chr(46) "com"
DMail := "drugwash" Chr(64) "yahoo" Chr(46) "com"
RMail1 := "meet_rajat" Chr(64) "gawab" Chr(46) "com"
DMail1 := "drugwash" Chr(64) "gmail" Chr(46) "com"
DwnUrl := "http://www" Chr(46) "autohotkey" Chr(46) "net/~Drugwash/SmartGUICreatorMod/"
RelUrl := ModHome
...
Loop, Parse, gridsizes, CSV
	Gui, 97:Add, Text, % "x" 5+65*(A_Index-1) " y" 67+65*((gridcount-1)//8) " w60 h16 0x200 Center BackgroundTrans gselimg vgs" A_Index, %A_LoopField% px
Loop, %skincount%
...
TBCtrls := "1|Button,2|Checkbox,14|Radio,6|Edit|0x18,17|Text,7|GroupBox,12|Picture,5|DropDownList,3|ComboBox,9|ListBox,10|ListView
...
Gui, 9:Add, Progress, x0 y0 w%WinWidth% h8 -Theme cRed Background%tc2% +E0x8 Hidden vSLP, 0
...
; somebody might erroneously use 'newline' as separator, so we correct this in the background
	...
	else StringReplace, TmpCtrlText, CtrlText, ``n, `n, A
...
niceText:
	...
	GuiControl, 98:%cOpt% -0x003000C0, Edit1
...
7ButtonOK:
	...
	Loop, Parse, NewOption, %A_Space%, %A_Space%
		...
		if Test in g
			if StrLen(cOpt) > 1
			{
			IniWrite, %cOpt%, %iniWork%, %CtrlNameCount%, gLabel
	...
	Control, Hide,, %CtrlNameCount%, ahk_id %MainWndID%
	GuiControl, 1:MoveDraw, %CtrlNameCount%
	Control, Show,, %CtrlNameCount%, ahk_id %MainWndID%
	tmpstrg := tmplist := ""
	Gui, 1:Default
7GuiClose:
...
askrows:
	...
	IfEqual, Ctrl2Add, Picture
		...
		Gui, 1:Add, %Ctrl2Add%, % "x" mX " y" mY " " CtrlOptions, %PicFile%
		}
	...
	IfEqual, Ctrl2Add, Picture
		JustCopy=Y	; allow pictures to be added at original size without automatic resizing
Alter:
...
resumeGenerate:
	...
	Else
		{
		FinalScript = %FinalScript%`; Generated using %appname% %version%`n
		i := AN ? GUIName : GeneratedWnd
		FinalScript = %FinalScript%Gui`, %GUICountA%Show`,%GuiPos% w%wW% h%wH%`, %i%`n
...
MoveGroup:
	...
	IfNotEqual, FirstTimeM, No
		...
		;ÐÞÕýÎÞ·¨Í¬Ê±ÒÆ¶¯¶à¸ö¿Ø¼þÎ»ÖÃµÄÎÊÌâ
...
ControlGetPos, cX, cY,,, %ctrl%, ahk_id %hwnd%
if (X != cX-1) OR (Y != cY-1)
	{
...
Static s=0, winlist="2,3,4,5,6,7,8,10,11,97,98,99", vislist
if wP != 2	; HTCAPTION
	return
s := !s
...
;		msgbox, Dropdown command #%cmdid%
		...
		if (flags & 0x20)	; HICF_LEAVING
			...
			MouseGetPos, x, y
			CoordMode, Mouse, Relative
			hBT := ShowBT(2, hTB99 "$" MenuWndID, txt, ttl, "C$" tc1 "$" tc2) ;" P$" x+15 "$" y+15)
			}
		last := new
		}
if (hwnd=hRB99)
