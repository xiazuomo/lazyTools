...
GetScriptLabels(ByRef s, bExternal = False) {
	...
	Loop {
		...
		If Not RegExMatch(t, "\Q" u "\E(?!\Q" v "\E)") {
			...
			nowval := ConvertPos(s,i,pre) + preval
			If bExternal {
				sLabels%sLabels0%_File := sPaths0
				sLabels%sLabels0%_Line := LineFromPosEx(s, nowval)
			} Else {
				sLabels%sLabels0%_File := 0
				sLabels%sLabels0%_Line := LineFromPos(nowval)
			}
...
GetScriptFunctions(ByRef s, bExternal = False) {
	...
	Loop {
		...
		i := RegExMatch(s, "m)(*ANYCRLF)(*UCP)^[[:blank:]]*\K[\w#_@\$\?\[\]]+"
						 . "(?=\(.*?\)(\s+\Q" u "\E.*?[\r\n]+)*?\s*\{)", t, i)
		...
		{   ;Increment counter
			...
			If bExternal {
				...
				sFuncs%sFuncs0%_Line := LineFromPosEx(s,nowval)
			} Else {
				sFuncs%sFuncs0%_File := 0
				sFuncs%sFuncs0%_Line := LineFromPos(nowval)
			}
			preval := nowval, pre := i
...
ConvertPos(Byref s, stop,start := 0){
	Global code_page
	;static lastval := 0
	;if (start = 0)
		;lastval := 0
	nasc := 0 ,cpb := (1+ !!A_IsUnicode)
	lps := stop -start
	loop , % lps
	{
		bx := NumGet( s , (start+A_index-1)*cpb, "UShort" )
		if bx > 255
			nasc ++
	}
	codebytes := 1
	if (code_page = 65001)
		codebytes := 3
	else if (code_page = 936)
		codebytes := 2
	
	;lastval += lps +(codebytes-1)*nasc
	return  lps +(codebytes-1)*nasc
}
...
Sci_GetText(hSci) {
	...
	code_page := ErrorLevel
	If ((A_IsUnicode And code_page != 1200) Or (!A_IsUnicode And code_page != 1252))
		sText := StrGet(&sText, "CP" . code_page)
